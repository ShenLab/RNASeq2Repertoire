...
open(FinIgHpos,'/ifs/scratch/c2b2/ys_lab/ahc2149/scripts/count-VDJ/Count-VDJ/coordinates/IGH.v37.YapingCoordinates_VDJonly.txt');
...
while (<mF>){
	...
	if ($indxP==0){
		...
		for (my $i=0; $i<=$#IgHuniq_ex; $i++){
			...
			if ($iWhile >= 0 && $iWhile < 9 ){
				push (@Jonly,$IgHanno[$iWhile][1]);
			} 
			elsif ($iWhile >= 9 && $iWhile < 36 ){
				push (@Donly,$IgHanno[$iWhile][1]);
			}
			elsif ($iWhile >= 36 && $iWhile < 159 ){
