...
open(FinIgHpos,'/ifs/scratch/c2b2/ys_lab/ahc2149/scripts/count-VDJ/Count-VDJ/coordinates/TRB.v37p8.YapingCoordinates_sort.3.txt');
...
# The coordinates in `YapingCoordinates` are given relative to start
# of the gene, but a "bump" to genomic coordinates is not needed, because
# we append the entire annotation from IMGT as an additional chromosome.
# Hence we filter to this TRB-only "chromosome", which makes all locations
# relative.
...
while (<mF>){
	...
	if ($indxP==0){
		...
		for (my $i=0; $i<=$#IgHuniq_ex; $i++){
			...
			if ($iWhile >= 69 && $iWhile < 83 ){
				push (@Jonly,$IgHanno[$iWhile][1]);		
			} 
			elsif ($iWhile >= 67 && $iWhile < 69 ){
				push (@Donly,$IgHanno[$iWhile][1]);			
			}
			elsif ($iWhile >= 0 && $iWhile < 67 ){
...
for (my $i=0; $i<=$#IgHuniq_ex; $i++){
    ...
    if ($iWhile >= 13 && $iWhile < 19 ){
        push (@Jonly,$IgHanno[$iWhile][1]);		
    } 
    elsif ($iWhile >= 19 && $iWhile < 46 ){
        push (@Donly,$IgHanno[$iWhile][1]);			
    }
    elsif ($iWhile >= 46 && $iWhile < 90 ){
        push (@Vonly,$IgHanno[$iWhile][1]);			
    }		
}
for (my $i=0; $i<=$#Vonly;$i++){
